        ...
        @row-click="rowClick">
          ...
          <el-table-column label="审核" :align="center">
            <template slot-scope="scope">
              <span class="table-button" @click="examineDlg(scope.row)">审核</span>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    examineDlg: function(row) {
      if (row === null || row.length === 0) {
        this.$message.warning('请选择要审核和课程')
      } else if (row.c_status === '3' && row.c_opinion !== '*重新申请') {
        this.$message.error('申请人未修改申请信息，不可重复审核')
      } else if (row.c_status === '2' && row.c_date !== null) {
        this.$message.error('课程已经安排,无法进行审核')
      } else {
